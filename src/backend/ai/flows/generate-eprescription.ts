...
// Fallback prescription generation when AI service is unavailable
function generateFallbackPrescription(input: GenerateEprescriptionInput): GenerateEprescriptionOutput {
  const formattedDate = new Date(input.consultationDate).toLocaleDateString('en-US', {
    year: 'numeric', month: 'long', day: 'numeric'
  });
  
  let prescriptionText = `E-PRESCRIPTION\n`;
  prescriptionText += `====================================\n\n`;
  
  prescriptionText += `DOCTOR INFORMATION:\n`;
  prescriptionText += `  Name: Dr. ${input.doctorName}\n`;
  prescriptionText += `  Specialization: ${input.doctorSpecialization || 'N/A'}\n`;
  prescriptionText += `  License ID: ${input.doctorLicenseId || 'N/A'}\n`;
  prescriptionText += `  Wallet: ${input.doctorWallet}\n\n`;
  
  prescriptionText += `PATIENT INFORMATION:\n`;
  prescriptionText += `  Patient ID: ${input.patientId}\n`;
  prescriptionText += `  Date of Consultation: ${formattedDate}\n\n`;
  
  prescriptionText += `PRESCRIPTION (Rx):\n`;
  input.medications.forEach((med, index) => {
    prescriptionText += `${index + 1}. Medication: ${med.name}\n`;
    prescriptionText += `   - Dosage: ${med.dosage}\n`;
    prescriptionText += `   - Frequency: ${med.frequency}\n`;
    if (med.notes) {
      prescriptionText += `   - Instructions: ${med.notes}\n`;
    }
    prescriptionText += `\n`;
  });
  
  prescriptionText += `DISCLAIMER:\n`;
  prescriptionText += `This is a digitally generated prescription. Please consult your pharmacist if you have any questions. This prescription is valid only when presented with a verifiable record on the MediChain platform.\n\n`;
  prescriptionText += `--- End of Prescription ---`;
...
export async function generateEprescription(
  ...
  try {
    // Use rate limiter to prevent quota exceeded errors
    return await aiRateLimiter.executeWithRateLimit(
      () => generateEprescriptionFlow(input),
      'e-prescription generation'
    );
  } catch (error) {
    console.error('AI service error in e-prescription generation:', error);
    
    // Handle Google AI service unavailability gracefully
    ...
    if (errorMessage && (
      ...
      console.log('Google AI service unavailable or rate limited, using fallback prescription generation');
      return generateFallbackPrescription(input);
    }
    
    // For other errors, re-throw with more context
    throw new Error(`Failed to generate e-prescription: ${(error as any)?.message || 'Unknown error'}`);
  }
}
