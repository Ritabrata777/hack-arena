...
export async function filterRelevantInsights(
  ...
  try {
    return await filterRelevantInsightsFlow(input);
  } catch (error) {
    ...
    if (
      ...
      // Heuristic fallback: consider insight relevant if it appears in the summary
      const relevant = input.consultationSummary.toLowerCase().includes(input.insight.toLowerCase());
      return { relevant, confidence: relevant ? 0.6 : 0.3, reason: 'Local fallback relevance check (string match).' };
    }
    throw error;
  }
}
